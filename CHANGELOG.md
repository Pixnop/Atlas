...
- `JoinPlayer` could fail intermittently with `InvalidOperationException: Collection was
  modified` from its internal inventory wait. The wait read
  `IPlayerInventoryManager.Inventories`, which copies the engine's unlocked inventory list on
  every call, and since 0.13.0 a throwing predicate faults its wait. The wait now checks the
  list's `Count` through `PlayerInventoryManager.Inventories`, which cannot throw. A scenario's
  own `Until` predicate still faults on any exception.
...
- `[AtlasScenario]` no longer dates its rollback behavior by internal design stage. The
