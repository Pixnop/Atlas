...
/// class the scenario fails with <see cref="Atlas.Api.AtlasSetupException"/> at invocation
/// time.</summary>
...
public sealed class AtlasScenarioAttribute : FactAttribute
{
    ...
    /// shutdown) fails the scenario with an <see cref="Atlas.Api.AtlasSetupException"/>, and a
    /// crash while booting the replacement surfaces as-is. There is no silent degrade, which is also why
    ...
    /// <see cref="Atlas.Api.AtlasIsolationException"/> carrying the degrade reason (a mod
    /// rollback hook failure, engine drift, or a generic capture/restore failure).</summary>
    ...
    /// surfacing as <see cref="Atlas.Api.ServerCrashedException"/>.</para>
