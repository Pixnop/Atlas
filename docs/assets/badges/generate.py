...
PALETTES = {
    "sepia": dict(
        bg="#F3E6C9", edge="#3B2A1A", ink="#3B2A1A", accent="#B0862B",
...
def titan_group(x: float, y: float, height: float, colors: dict[str, str] | str) -> str:
...
# --- formats -------------------------------------------------------------------
def build_flat(p: dict) -> tuple[str, float, float]:
    h = 20.0
    ...
    w = label_w + msg_w
    body = [
        f'<clipPath id="clip"><rect width="{w:.2f}" height="{h}" rx="3"/></clipPath>',
    ...
    mark, mark_actual_w = titan_group(label_w + pad, 3, 14, p["mark_fg"])
    body.append(mark)
...
def build_plaque(p: dict) -> tuple[str, float, float]:
    w, h = 220.0, 60.0
...
def build_square(p: dict) -> tuple[str, float, float]:
    w = h = 160.0
...
def build_seal(p: dict) -> tuple[str, float, float]:
    w = h = 112.0
    cx, cy = w / 2, h / 2
    mark_w = titan_width(38)
    mark, _ = titan_group(cx - mark_w / 2, cy - 19, 38, p["titan"])
    ring_path = arc_path(cx, cy, 44, -125, 125)
    body = [
        f'<circle cx="{cx}" cy="{cy}" r="{w/2 - 1}" fill="{p["bg"]}"/>',
        f'<circle cx="{cx}" cy="{cy}" r="{w/2 - 2.2}" fill="none" '
        f'stroke="{p["edge"]}" stroke-width="1.6"/>',
        f'<circle cx="{cx}" cy="{cy}" r="{w/2 - 6}" fill="none" '
        f'stroke="{p["edge"]}" stroke-width="0.6"/>',
        f'<defs><path id="ring" d="{ring_path}"/></defs>',
        textpath_el("ring", 10.5, p["ink"], "TESTED WITH ATLAS", spacing=0.6),
        mark,
        f'<circle cx="{cx}" cy="{cy + 27}" r="1.8" fill="{p["accent"]}"/>',
    ]
    return svg_doc(w, h, "".join(body)), w, h


def build_wide(p: dict) -> tuple[str, float, float]:
    w, h = 600.0, 100.0
...
def render(fmt: str, palette: str) -> None:
    p = PALETTES[palette]
    raw_svg, w, h = FORMATS[fmt](p)

    raw_path = OUT / f".raw-{fmt}-{palette}.svg"
